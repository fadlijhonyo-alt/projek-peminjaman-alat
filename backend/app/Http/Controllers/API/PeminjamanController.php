        ...
        try { 
            $peminjaman = DB::transaction(function () use ($request) { 
                ...
                foreach ($request->items as $item) { 
                    ...
                    $alat = 
Alat::lockForUpdate()->findOrFail($item['alat_id']); 
                    if ($alat->stok < $item['jumlah']) { 
                        throw new Exception("Stok alat 
'{$alat->nama_alat}' tidak mencukupi. Sisa stok: {$alat->stok}"); 
                    } 
                    DetailPinjam::create([ 
    ...
    { 
        ...
        if ($user->role === 'peminjam' && $peminjaman->user_id !== 
$user->id) { 
     ...
     public function update(StorePeminjamanRequest $request, Peminjaman 
$peminjaman): JsonResponse 
    { 
        $user = auth()->user(); 
...
Alat::lockForUpdate()->findOrFail($item['alat_id']); 
                    ...
                    if ($alat->stok < $item['jumlah']) { 
                        throw new Exception("Stok alat 
'{$alat->nama_alat}' tidak mencukupi."); 
                    } 
            ...
            return response()->json([ 
                ...
                'data' => new 
            PeminjamanResource($peminjaman->load(['user', 'detailPinjam.alat'])) 
            ]); 
...
$user->id) { 
        ...
        if ($peminjaman->status !== 'diajukan') { 
            return response()->json(['message' => 'Peminjaman tidak dapat 
dibatalkan.'], 400); 
        } 
        ...
        return response()->json([ 
            'message' => 'Permohonan peminjaman berhasil dibatalkan dan 
dihapus.' 
        ]);
    }
 ...
 public function approve(Peminjaman $peminjaman): JsonResponse 
    { 
        if ($peminjaman->status !== 'diajukan') { 
            return response()->json([ 
                'message' => "Persutujuan gagal. Status saat ini: 
{$peminjaman->status}." 
            ], 400); 
        } 
        ...
        try { 
            DB::transaction(function () use ($peminjaman) { 
                ...
                foreach ($peminjaman->detailPinjam as $detail) { 
                    ...
                    $alat = 
Alat::lockForUpdate()->findOrFail($detail->alat_id); 
                    
                    if ($alat->stok < $detail->jumlah) { 
 
 
                        throw new Exception("Persetujuan gagal. Stok alat 
'{$alat->nama_alat}' mendadak tidak mencukupi."); 
                    } 
